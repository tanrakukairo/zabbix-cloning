...
ホストに`ZC_UUID`がなければ自動生成します。LLDで自動登録されたホスト（`flags=4`）はUUID付与とmasterデータ取得の対象外です。
...
- VERSIONとDATAは指定した単一DBへ保存（デフォルト: DB 0）
- キー: `ZC_VERSION:<VERSION_ID>`、`ZC_DATA:<VERSION_ID>`
- DATAはbzip2圧縮
- デフォルトポート: `6379`
...
| `--delete.host` | ストアにないホストを削除。LLDで自動登録されたホスト（`flags=4`）は対象外 |
...
| `-sd`, `--store.db DB` | Redis DB番号。デフォルト0 |
...
| `store_connect.redis_db` | Redis DB番号。デフォルト0 |
...
Go版と[Python旧版](prototype/readme.md)は、fileとDynamoDBのデータ形式を共有します。
RedisはGo版が単一DB形式、Python旧版がDB 0/DB 1分割形式のため互換性がありません。
DB 0/DB 1に分割していた旧Redis形式は自動移行されないため、更新後はデータを再作成してください。
