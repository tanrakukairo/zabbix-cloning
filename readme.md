...
Go版はZabbix 6.0から7.0までに対応します。Zabbix 6.0未満を扱う場合は
[Python旧版](prototype/readme.md)を使用してください。
...
開発版およびZabbix 7.2以降は現在の対応範囲に含みません。
...
PSK、CheckNow、初期化、バージョンマクロ更新も実行対象から除外されます。

masterではストアへ新しいバージョンを保存しません。CheckNowの適用待機時間も省略します。
終了時に、実行せず記録したAPIメソッド別件数をログへ表示します。

replicaでdry-runを実行すると、最初のGET結果を基に仮想状態を作成します。`--initialize`で
...
初回は削除オプションなしで差分を確認してください。`--initialize`指定時は削除オプションより初期化が優先されます。
...
- Zabbix 6.0未満。必要な場合は[Python旧版](prototype/readme.md)を使用してください。
