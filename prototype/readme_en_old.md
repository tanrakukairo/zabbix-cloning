...
> The primary implementation now lives in [`go-lang/`](go-lang/). The former
> Python implementation was moved to [`prototype/`](prototype/) and remains as
> the Zabbix 4.x/5.x compatibility implementation.
>
> ```sh
> cd go-lang
...
> ```
>
> The Go implementation targets Zabbix 6.0 and newer and shares the file,
> Redis, and DynamoDB formats with the Python prototype. Some detailed sections
> below still describe prototype-only compatibility behavior.
# Contents
- [Overview](#overview)
...
- [Specification](#specification)
    ...
    - [Configuration](#configuration)
        ...
        - [Operation Settings](#operation-settings)
            ...
            - [Delete Existing Hosts](#delete-existing-hosts)
            - [Delete Existing API-managed Settings](#delete-existing-api-managed-settings)
...
## Environment
The Go implementation targets Windows and Linux. See
[`prototype/README.md`](prototype/README.md) for the Python environment.

## Requirement

* Go implementation: Zabbix 6.0+ and Go 1.23+
* Python prototype: Zabbix 4.0+ and Python 3.6+

excluding development versions.

Go dependencies are downloaded by `go mod download` or `go build`. See the
prototype README for its Python dependencies.
...
```sh
go-lang/bin/zc clone --config.file ./file
```
...
Configurations that are not in master node and exist in worker nodes can be deleted by specifying delete options.
...
go-lang/bin/zc clone --role master \
            --node master-zabbix \
            --store-type file \
...
go-lang/bin/zc clone --role worker \
            --node worker-zabbix \
            --store-type file \
...
```sh
    ...
    # Delete hosts that do not exist in master node data
    --delete.host

    # Delete API-managed settings that do not exist in master node data
    --delete.api
...
go-lang/bin/view showdata --version xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx
```
##### required
```sh
...
#### Change Password
    ...
    CONFIG: {"update_password": true|false}
    default: NO
...
#### Initialize
    COMMAND: --initialize
    CONFIG: {"initialize": true|false}
    default: NO

Delete all Zabbix configuration on worker nodes.<br>
This operation runs only when this option is enabled, even if "{$ZC_VERSION}" is not in globalmacro on worker nodes.

#### Use IPaddress
    COMMAND: --useip
    CONFIG: {"useip": true|false}
    default: NO
...
#### Force Update Host Configuration
    ...
    CONFIG: {"force_host_update": true|false}
    default: NO
...
#### Delete Existing Hosts
    COMMAND: --delete.host
    CONFIG: {"delete_host": true|false}
    default: false

Hosts that do not exist in master node data are deleted from worker nodes.

#### Delete Existing API-managed Settings
    COMMAND: --delete.api
    CONFIG: {"delete_api": true|false}
    default: false

API-managed settings that do not exist in master node data are deleted from worker nodes.

#### Skip Import/Export Zabbix Templates
    COMMAND: --skip.template
    CONFIG: {"skip_template": true|false}
    default: NO

If this argument is specified, template export from master node and template import to worker nodes is not executed.

#### Skip Import Zabbix Hosts
    COMMAND: --skip.host
    CONFIG: {"skip_host": true|false}
    default: false

If this argument is specified on worker nodes, host import is not executed.
...
#### Execute CheckNow
    ...
    CONFIG: {"checknow_execute": true|false}
    default: NO
...
#### Allow Cloning Super Admin's Settings
    CONFIG: {"cloning_super_admin": true|false}
    default: false
...
delete.host/delete.api separates deletion targets
